const { TikTokCrawler } = require('./tiktok-crawl');
const { GoogleSheetsManager } = require('./tiktok-sheets');
...
async function main() {
    console.log('═══════════════════════════════════════════');
    console.log('   K-POP Metrics Crawler');
    console.log('═══════════════════════════════════════════\n');
    ...
    try {
        ...
        if (metrics.length === 0) {
            console.log('\n⚠️ No videos scraped!');
            console.log('💡 Tips:');
            console.log('   - Run "npm run diagnostic" to analyze the page');
        ...
        const backupFile = `./data/metrics_${timestamp}.json`;
        ...
        if (error.message.includes('No videos found')) {
            console.error('\n💡 Troubleshooting:');
            console.error('   1. Run "npm run diagnostic" to analyze the page');
...
if (args.includes('--help') || args.includes('-h')) {
    console.log(`
K-POP Metrics Crawler

Usage:
  node index.js           Run full crawl and update sheets
  node index.js --crawl   Only crawl (don't update sheets)
  node index.js --test    Test Google Sheets connection

Setup:
  1. Download credentials.json from Google Cloud Console
  2. Set SPREADSHEET_ID in config.js
